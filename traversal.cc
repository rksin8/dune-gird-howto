...
void traversal (G& grid)
{
  ...
  count=0;
  std::cout << "Iterating using for loop range base:" << std::endl;
  for(const auto& cell : elements(leafView))
  {
    auto gt = cell.type();
    std::cout << "visiting leaf " << gt                /*@\label{tc:print}@*/
              << " with first vertex at " << cell.geometry().corner(0)
              << std::endl;
    count++;                                           /*@\label{tc:count}@*/
  }
  std::cout << "there are/is " << count << " leaf element(s)" << std::endl;
  ...
  for (int level=0; level<=grid.maxLevel(); level++)
  {
    ...
    std::cout << "Using c++11 range-based for loop:" << std::endl;
    count=0;
    for(const auto& cell: elements(levelView))
    {
      auto gt = cell.type();
      std::cout << "visiting " << gt
                << " with first vertex at " << cell.geometry().corner(0)
