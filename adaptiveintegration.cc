...
//      value += integrateEntity(*it,f,highorder);           /*@\label{aic:int1}@*/
    ...
    for(const auto& cell:elements(gridView))
      value+=integrateEntity(cell,f,highorder);
    ...
    /* C++ range-based for loop to iterates through all the elements */
    for (const auto& cell : elements(gridView))
    {
      // error on this entity
      double lowresult=integrateEntity(cell,f,loworder);
      double highresult=integrateEntity(cell,f,highorder);
      double error = std::abs(lowresult-highresult);
      ...
      // error on father entity
      double fatherlowresult=integrateEntity(*(cell.father()),f,loworder);
      double fatherhighresult=integrateEntity(*(cell.father()),f,highorder);
      double fathererror = std::abs(fatherlowresult-fatherhighresult);
...
//   }                                                  /*@\label{aic:mark1}@*/
    ...
    for(const auto& cell:elements(gridView))
    {
      ...
      double highresult=integrateEntity(cell,f,highorder);
      double error = std::abs(lowresult-highresult);
      if (error>kappa) grid.mark(1,cell);
    }
